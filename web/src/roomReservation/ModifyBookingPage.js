//ModifyBookingPage.js
import React, { useState, useEffect } from 'react';
import { Container, Button, TextField, Typography, InputLabel, Select, MenuItem } from '@mui/material';
...
const ModifyBookingPage = ({ editData }) => {
  ...
  useEffect(() => {
    // Set the initial booking details when editData changes
    setBookingDetails({
      roomType: editData.roomType || '',
      customerName: editData.customerName || '',
      phoneNumber: editData.phoneNumber || '',
  ...
  const handleChange = (e) => {
    ...
    if (fieldName === 'phoneNumber' && trimmedValue !== '' && !/^\d+$/.test(trimmedValue)) {
      Swal.fire({
        icon: 'error',
        title: 'Invalid Input',
    ...
    setBookingDetails((prevDetails) => ({ ...prevDetails, [name]: trimmedValue }));
  };
  ...
  return (
   ...
   <div style={{ display: 'flex', justifyContent: 'space-between', backgroundColor: '#838d97', alignItems: 'center', borderRadius: '10px' }}>
    ...
    {showForm && (
      <StyledForm >
    <StyledH1 message="Modify Booking" className="display-4" />
   ...
   <div className="mb-3">
        ...
        <StyledButtonContainer>
          <StyledButton type="submit" variant="contained" color="primary" className="mb-3" onClick={handleModifyBooking}>
            Update Booking
          </StyledButton>
        </StyledButtonContainer>
        </StyledForm>
